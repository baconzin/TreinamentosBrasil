...
function renderCatalog(){
  ...
  function block(c, alt){
    ...
    details.appendChild(summary); details.appendChild(panel);
    return details;
  }
...
function injectJSONLD(){
  ...
  const courses = {
    '@context':'https://schema.org', '@type':'ItemList', itemListElement: catalog.map((c,i)=>({
      '@type':'Course', position:i+1, name:c.title, description:c.summary,
      provider: { '@type':'Organization', name: brand.name }
    }))
  };
  document.getElementById('courses-json').textContent = JSON.stringify(courses);
}
